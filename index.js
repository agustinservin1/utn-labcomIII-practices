...
const getMoviesByDirector = (watchlist) => {
    return watchlist
           .filter(movie => movie.Director === 'Christopher Nolan')
           .map(movie => movie.Title);
};

const nolanMovies = getMoviesByDirector(watchList);
console.log(nolanMovies); 


//EJERCICIO 2 : 

//Probé también de castear el dato contenido en imdbRating que viene como tipo string a float, en este caso la comparación no generaba conflicto y funcionaba de ambas maneras.

const getHighRatedMovies = (watchlist) => {
    return watchlist
           .filter(movie => movie.imdbRating > 8.0)  
           .map(movie => movie.Title); 
};

const highRatedMovies = getHighRatedMovies(watchList);
